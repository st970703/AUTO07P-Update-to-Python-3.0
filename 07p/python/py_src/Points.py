...
class auxfnDBclass(object):
    ...
    def removeAuxFn(self, auxfnName):
        flagdelete = None
        for k, v in list(self.auxnames.items()):
            if v == auxfnName:
                flagdelete = k
                break
...
def invertMap(themap):
    ...
    if isinstance(themap, dict):
        return dict([(v,k) for (k,v) in list(themap.items())])
...
class Point(object):
    ...
    def __init__(self, kwd=None, **kw):
        ...
        if 'coorddict' in kw:
            ...
            for c, v in list(kw['coorddict'].items()):
    ...
    def _infostr(self, verbose=0):
        ...
        if verbose == 0:
            ...
            for c in self.coordnames:
                ...
                if c != self.coordnames[-1]:
                    outputStr += "\n"
            for label, infodict in list(self.labels.items()):
    ...
    def __getitem__(self, ix):
        ...
        if isinstance(ref1, (list, ndarray, _int_types)):
            ...
            if (s3 > 0 and s1 >= len(self)) or (s3 < 0 and s2 >= len(self)):
                raise ValueError("Slice index out of range")
            ca = take(self.coordarray, list(range(s1, s2, s3)), axis=1)
            try:
                ci = take(self.indepvararray, list(range(s1, s2, s3)),axis=0)
            except (IndexError, AttributeError, ValueError):
                # non-parameterized pointset
                pass
            cl = self.labels[ref1]
...
class PointInfo(object):
    ...
    def __init__(self, ptlabels=None):
        if ptlabels is None:
            ...
            self.by_label = DefaultDict({})
            self.by_index = DefaultDict({})
            for k, v in list(ptlabels.items()):
                ...
                if isinstance(v, str):
                    ...
                    for label, infodict in list(v.items()):
    ...
    def mapIndices(self, ixMapDict):
        ...
        for ix, rest in list(self.by_index.items()):
    ...
    def mapNames(self, themap):
        ...
        new_by_index = {}
        for ix, labdict in list(self.by_index.items()):
    ...
    def update(self, key1, key2=None, info=None):
        if isinstance(key1, PointInfo):
            if key2 is None and info is None:
                for k, v in list(key1.by_index.items()):
