...
import AUTOutil
import AUTOExceptions, parseC, Points
...
def reverse_type_translation(type):
    ...
    for k,v in list(type_translation_dict.items()):
...
class BDPoint(Points.Point):
    def __init__(self, p, branch=None, idx=None, pt=None):
        if isinstance(p['coordarray'], list):
            ...
            self.name_map = dict(list(zip(p['coordnames'],
                                     list(range(len(p['coordnames']))))))
        else:
...
class AUTOBranch(parseBMixin, Points.Pointset):
    def __init__(self,input=None,prevline=None,coordnames=[]):
        ...
        if isinstance(input,AUTOBranch):
            for k,v in list(input.__dict__.items()):
                self.__dict__[k] = v
    ...
    def _gettypelabel(self,idx):
        for k,v in list(self.labels[idx].items()):
            if "LAB" in v:
                return k,v
        return 'No Label', {}
    ...
    def toArray(self):
        return list(map(list,list(zip(*self.coordarray))))
    ...
    def write(self, output, columnlen=19):
        ...
        for i in range(len(data[0])):
            ...
            if i in self.labels.by_index:
                for k,label in list(self.labels[i].items()):
                    if "LAB" in label:
